...
export default function WorkshopCard({ workshop }) {
  ...
  const handleRegister = () => {
    ...
    const registrationUrl = "https://docs.google.com/forms/d/e/1FAIpQLSd2z_jmy6CC9vZ7_yEtyvsSEcNNiTOzv9JY_VqCLftbQKEHAA/viewform?fbclid=PAVERFWAOmRwxleHRuA2FlbQIxMABzcnRjBmFwcF9pZA8xMjQwMjQ1NzQyODc0MTQAAad3miAUSgyZVTfctq1YDcGY4L4x_XKnHjtTAOFBqfRsA-0qb9qIiEDcLkWLEQ_aem_vkMU6dZk9A7pjHfsHJrJYQ"; // Replace with actual URL
  ...
  return (
    ...
    ">
      ...
      {open && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fade-in" role="dialog" aria-modal="true">
          <div
            className="
          ...
          >
            ...
            <div className="absolute inset-0 rounded-3xl pointer-events-none bg-gradient-to-br from-indigo-200/40 via-white/10 to-rose-200/40 opacity-40 blur-xl"></div>

            {/* Close button */}
            <button
              onClick={() => setOpen(false)}
              className="absolute right-5 top-5 text-slate-600 hover:text-slate-900 transition-colors text-xl"
            >
              ✕
            </button>
            ...
            <p className="mt-4 text-sm text-slate-600"><strong className="font-semibold">Mentors :</strong> {mentors}</p>

            <div className="mt-4 text-indigo-700 font-semibold text-sm">{price} {currency || "DA"}</div>
            ...
            <div className="mt-7 flex gap-3">
              <button
                onClick={handleRegister}
                className="px-5 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-700 text-white shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 text-sm font-semibold"
              >
              ...
              <button
                onClick={() => setOpen(false)}
                className="px-5 py-3 rounded-xl bg-white/70 backdrop-blur-sm border border-slate-200 text-slate-700 shadow-sm hover:shadow-md hover:scale-105 transition-all duration-300 text-sm"
              >
                Fermer
              </button>
            </div>
      ...
      {notOpen && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" role="dialog" aria-modal="true">
          <div className="bg-white/80 backdrop-blur-lg rounded-2xl max-w-sm w-full p-6 text-center shadow-xl border border-white/50 animate-fade-up">
            ...
            <p className="text-sm text-slate-600 mb-4">L’inscription n’est pas encore ouverte - restez branché.</p>

            <button
              onClick={() => setNotOpen(false)}
              className="px-5 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-xl shadow hover:shadow-lg hover:scale-105 transition-all duration-300"
            >
              Fermer
            </button>
          </div>
